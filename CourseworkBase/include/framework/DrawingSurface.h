...
public:
	...
	{
		m_pSDLSurface = SDL_CreateSurface(iWidth, iHeight, SDL_PIXELFORMAT_ARGB8888);
	}
	...
	*/
	...
	void fillSurface(unsigned int uiPixelColour)
	{
		...
		rect.x = 0;
		rect.y = 0;
		rect.w = getSurfaceWidth();
		rect.h = getSurfaceHeight();
		mySDLTempUnlockSurface();
		::SDL_FillSurfaceRect(m_pSDLSurface, &rect, 0xff000000 | uiPixelColour); // Note: set full alpha in case we copy it later
...
public:
	...
	void mySDLUpdateTexture(SDL_Texture *pTexture)
	{
		SDL_UpdateTexture(pTexture, NULL, m_pSDLSurface->pixels, m_pSDLSurface->pitch);
	}
