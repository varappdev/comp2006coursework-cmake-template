...
public:
	...
	*/
	...
	{
		float fx, fy;
		m_iCurrentButtonStates = SDL_GetMouseState(&fx, &fy);
		m_iCurrentMouseX = (int)fx;
		m_iCurrentMouseY = (int)fy;
	}
...
public:
	...
	*/
	...
	/* This function is used to map from an SDL keycode to a scancode index for key tracking.
	In SDL3, keycodes are no longer small contiguous integers, so we use scancodes for the internal array.
	*/
	static int symbolToKeycode(int iSym)
	{
		SDL_Scancode sc = SDL_GetScancodeFromKey((SDL_Keycode)iSym, NULL);
