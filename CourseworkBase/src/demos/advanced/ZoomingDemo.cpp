...
int ZoomingDemo::virtInitialiseObjects()
{
	...
	storeObjectInArray(1, new DragableImageObject( 200, 200, this, "resources/demo2a.png", false));
	storeObjectInArray(2, new SimpleFlashingDragableImageObject(400, 200, this, "resources/demo2a.png", "resources/demo2b.png", false));
...
void ZoomingDemo::virtDrawStringsUnderneath()
{
	...
	char buf[128];
	snprintf(buf, sizeof(buf), "Changing random %6d", rand() );
...
void ZoomingDemo::virtDrawStringsOnTop()
{
	// Build the string to print
	char buf[128];
	snprintf(buf, sizeof(buf), "Time %6d", rand());
...
void ZoomingDemo::virtKeyDown(int iKeyCode)
{
	...
	case SDLK_DOWN:
		...
		redrawDisplay();
		break;
	case SDLK_EQUALS: // + key zooms in (stretch)
	case SDLK_PLUS:
		m_filterScaling.stretch();
		redrawDisplay();
		break;
	case SDLK_MINUS: // - key zooms out (compress)
		m_filterScaling.compress();
		redrawDisplay();
		break;
	case SDLK_SPACE: // Space resets both translation and zoom back to initial state
		m_filterTranslation.setOffset(0, 0);
		m_filterScaling.setStretch(0, 0);
		redrawDisplay();
		break;
	}
}
