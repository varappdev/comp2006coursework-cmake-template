...
void BaseEngine::virtCreateWindows(const char* szCaption)
{
	...
	m_pWindow = SDL_CreateWindow(szCaption,
	...
	SDL_SetWindowPosition(m_pWindow, 10, 40); // Position the window after creation
	...
	m_pTexture = SDL_CreateTexture(m_pRenderer, SDL_PIXELFORMAT_ARGB8888, SDL_TEXTUREACCESS_STREAMING, m_iWindowWidth, m_iWindowHeight);
}
...
int BaseEngine::initialise(const char* szCaption, int iWindowWidth, int iWindowHeight, const char* szFontName, int iFontSize)
{
	...
	// Initialize SDL's subsystems - in SDL3, SDL_Init returns true on success, false on failure
	...
	m_pKeyStatus = new bool[1 + SDL_SCANCODE_COUNT];
	memset( m_pKeyStatus,0,sizeof( bool )*(1 + SDL_SCANCODE_COUNT) );
...
bool BaseEngine::handleEvent(SDL_Event* pEvent)
{
	...
	if (firstClick && (pEvent->type == SDL_EVENT_MOUSE_BUTTON_DOWN) )
	{
		firstClick = false;
	...
	case SDL_EVENT_KEY_DOWN:
		//fprintf(fdebug,"Key down: %d, '%c'\n", pEvent->key.scancode, pEvent->key.key);
		m_pKeyStatus[pEvent->key.scancode] = true;
		virtKeyDown(pEvent->key.key);
		if (m_bNotifyObjectsAboutKeys)
			std::for_each(m_vecDisplayableObjects.begin(), m_vecDisplayableObjects.end(), [=](DisplayableObject* pdo) { if (pdo == nullptr) return; pdo->virtKeyDown(pEvent->key.key); });
		break;

	case SDL_EVENT_KEY_UP:
		//fprintf(fdebug, "Key up: %d, '%c'\n", pEvent->key.scancode, pEvent->key.key);
		m_pKeyStatus[pEvent->key.scancode] = false;
		virtKeyUp(pEvent->key.key);
		if (m_bNotifyObjectsAboutKeys)
			std::for_each(m_vecDisplayableObjects.begin(), m_vecDisplayableObjects.end(), [=](DisplayableObject* pdo) { if (pdo == nullptr) return; pdo->virtKeyUp(pEvent->key.key); });
		break;

	case SDL_EVENT_MOUSE_MOTION:
		//fprintf(fdebug, "SDL_EVENT_MOUSE_MOTION: %d, '%d'\n", (int)pEvent->motion.x, (int)pEvent->motion.y );
		...
		m_iCurrentMouseX = m_pForegroundSurface->convertRealToVirtualXPosition((int)pEvent->motion.x);
		m_iCurrentMouseY = m_pForegroundSurface->convertRealToVirtualYPosition((int)pEvent->motion.y);
		virtMouseMoved(m_iCurrentMouseX, m_iCurrentMouseY);
		if (m_bNotifyObjectsAboutMouse)
			std::for_each(m_vecDisplayableObjects.begin(), m_vecDisplayableObjects.end(), [this](DisplayableObject* pdo) { if (pdo == nullptr) return; pdo->virtMouseMoved(m_iCurrentMouseX, m_iCurrentMouseY); });
		break;

	case SDL_EVENT_MOUSE_BUTTON_DOWN:
		//fprintf(fdebug, "SDL_EVENT_MOUSE_BUTTON_DOWN: %d, '%d'\n", (int)pEvent->button.x, (int)pEvent->button.y);
		virtRealMouseDown(pEvent->button.button, (int)pEvent->button.x, (int)pEvent->button.y);
		// Note: If you are not using a filter this just assigns the motion.x to the m_iCurrentMouseX. If using a filter it may convert the point. See filter at top of DrawingSurface if you really need to know what this is doing.
		m_iMouseXClickedDown = m_pForegroundSurface->convertRealToVirtualXPosition((int)pEvent->button.x);
		m_iMouseYClickedDown = m_pForegroundSurface->convertRealToVirtualYPosition((int)pEvent->button.y);
		virtMouseDown(pEvent->button.button, m_iMouseXClickedDown, m_iMouseYClickedDown);

		if (m_bNotifyObjectsAboutMouse)
			std::for_each(m_vecDisplayableObjects.begin(), m_vecDisplayableObjects.end(), [pEvent,this](DisplayableObject* pdo) { if (pdo == nullptr) return; pdo->virtMouseDown(pEvent->button.button, m_iMouseXClickedDown, m_iMouseYClickedDown); });
		...
		{ // No object was clocked, so notify that the background was clicked
			virtBackgroundMouseDown(pEvent->button.button, (int)pEvent->button.x, (int)pEvent->button.y, m_iMouseXClickedDown, m_iMouseYClickedDown);
		}
		break;

	case SDL_EVENT_MOUSE_BUTTON_UP:
		//fprintf(fdebug, "SDL_EVENT_MOUSE_BUTTON_UP: %d, '%d'\n", (int)pEvent->button.x, (int)pEvent->button.y);
		virtRealMouseUp(pEvent->button.button, (int)pEvent->button.x, (int)pEvent->button.y);
		// Note: If you are not using a filter this just assigns the motion.x to the m_iCurrentMouseX. If using a filter it may convert the point. See filter at top of DrawingSurface if you really need to know what this is doing.
		m_iMouseXClickedUp = m_pForegroundSurface->convertRealToVirtualXPosition((int)pEvent->button.x);
		m_iMouseYClickedUp = m_pForegroundSurface->convertRealToVirtualYPosition((int)pEvent->button.y);
		virtMouseUp(pEvent->button.button, m_iMouseXClickedUp, m_iMouseYClickedUp);
		if (m_bNotifyObjectsAboutMouse)
			std::for_each(m_vecDisplayableObjects.begin(), m_vecDisplayableObjects.end(), [pEvent,this](DisplayableObject* pdo) { if (pdo == nullptr) return; pdo->virtMouseUp(pEvent->button.button, m_iMouseXClickedUp, m_iMouseYClickedUp); });
		//redrawDisplay();
		break;

	case SDL_EVENT_MOUSE_WHEEL:
		virtMouseWheel((int)pEvent->wheel.x, (int)pEvent->wheel.y, pEvent->wheel.which, pEvent->wheel.timestamp);
		break;
...
void BaseEngine::virtRenderScreen(void)
{
	...
	m_pForegroundSurface->mySDLUpdateTexture(m_pTexture);
...
{
	SDL_Scancode sc = SDL_GetScancodeFromKey((SDL_Keycode)iKeyCode, NULL);
	if (sc >= SDL_SCANCODE_COUNT) return false;
	return m_pKeyStatus[sc];
}
