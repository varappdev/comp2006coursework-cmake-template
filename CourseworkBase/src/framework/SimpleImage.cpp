	...
	int iLeftInImage, int iTopInImage, int iWidthInImage, int iHeightInImage)
{
	...
	{ // Need a rescale first
		...
		{ // Need a bigger surface as this does not exist or is not big enough
			if (m_pRescaleSurface != nullptr)
				SDL_DestroySurface(m_pRescaleSurface);
			m_pRescaleSurface = SDL_CreateSurface(newWidth, newHeight, SDL_PIXELFORMAT_ARGB8888);
		}
		else
		{
		...
		::SDL_BlitSurfaceScaled(src, &rectSrc, m_pRescaleSurface, &rectTemp, SDL_SCALEMODE_NEAREST);
	...
	::SDL_SetSurfaceClipRect(dest, &rectClip);
